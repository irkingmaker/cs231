...
def conv_forward_naive(x, w, b, conv_param):
    ...
    h_=int(1+(x.shape[2]+2*pad - w.shape[2])/stride)
    w_=int(1+(x.shape[3]+2*pad - w.shape[3])/stride)
    out= np.zeros((x.shape[0],w.shape[0],h_,w_))
    x_in=np.pad(x,((0,0),(0,0),(pad,pad),(pad,pad)),'constant')
    for n in range(0,x.shape[0]):
        for f in range(0,out.shape[1]):
            hs=0
            for h_o in range(0,h_):
                ws=0
                ho=hs+w.shape[2]
                for w_o in range(0,w_):
                    wo=ws+w.shape[3]
                    out[n,f,h_o,w_o]=np.sum(np.multiply(x_in[n,:,hs:ho,ws:wo],w[f]))+b[f]
                    ws+=stride
                hs+=stride
    pass
    ###########################################################################
...
def conv_backward_naive(dout, cache):
    ...
    dout_pad = np.pad(dout,((0,0),(0,0),(pad_out,pad_out),(pad_out,pad_out)),'constant')
    for i in range(0,dout.shape[0]):
      for hh in range(0,dout.shape[2]):
        for ww in range(0,dout.shape[3]):
...
def max_pool_forward_naive(x, pool_param):
    ...
    for hh in range(0,h_out):
      for ww in range(0,w_out):
...
def max_pool_backward_naive(dout, cache):
    ...
    dx=np.zeros_like(x)
    ph,pw,stride=pool_param['pool_height'],pool_param['pool_width'],pool_param['stride']
    h_out,w_out=int(1+(x.shape[2]-ph)/stride),int(1+(x.shape[3]-pw)/stride)
    for hh in range(0,h_out):
      for ww in range(0,w_out):
        ...
        for l,k in enumerate(x_max.ravel()):
          dx[int(l/x.shape[1]),int(l%x.shape[1]),hh*stride +int(k/pw),ww*stride + int(k%pw)] += dout[int(l/x.shape[1]),int(l%x.shape[1]),hh,ww]
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
